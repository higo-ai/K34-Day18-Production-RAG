...
def load_documents(data_dir: str = DATA_DIR) -> list[dict]:
    ...
    for fp in sorted(glob.glob(os.path.join(data_dir, "*.pdf"))):
        ...
        if text:
            ...
            print(f"  [!] Bỏ qua {os.path.basename(fp)}: PDF scan ảnh, không có text layer (cần OCR).")
...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    # Split text thành sentences, bỏ qua chuỗi rỗng
    ...
    chunks = []
    if len(sentences) == 1:
        chunks.append(Chunk(text=sentences[0], metadata={**metadata, "strategy": "semantic"}))
        return chunks
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for para in paragraphs:
        if current_parent_len + len(para) + 2 > parent_size and current_parent_paras:
            ...
            for sentence in sentences:
                if current_child_len + len(sentence) + 1 > child_size and current_child_sentences:
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    for part in parts:
        ...
        if re.match(r'^#{1,3}\s+', part):
            content_str = "".join(current_content_parts).strip()
            if content_str or current_header:
                chunk_text = f"{current_header}\n\n{content_str}".strip() if current_header else content_str
                section_name = current_header.strip("# ").strip() if current_header else "Introduction"
                chunks.append(Chunk(
                    text=chunk_text,
